...
import { DeleteTagUseCaseDTO, DeleteTagUseCaseResponseDTO } from '../../domain/dtos/usecases/Tag.usecase.dto';
...
export class DeleteTagUseCase implements UseCaseInterface {
    ...
    async execute({
        id,
        user_email,
    }: DeleteTagUseCaseDTO): Promise<DeleteTagUseCaseResponseDTO> {
        ...
        const tag = await this.tagRepository.delete({
            id,
            updated_by: user.id,
        });
