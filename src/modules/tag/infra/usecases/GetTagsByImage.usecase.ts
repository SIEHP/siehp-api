...
import { GetTagsByImageUseCaseDTO, GetTagsByImageUseCaseResponseDTO } from '../../domain/dtos/usecases/Tag.usecase.dto';

@Injectable()
export class GetTagsByImageUseCase implements UseCaseInterface {
    constructor(
    ...
    async execute({
        image_id,
        user_email,
    }: GetTagsByImageUseCaseDTO): Promise<GetTagsByImageUseCaseResponseDTO[]> {
        ...
        const tags = await this.tagRepository.findTagsByImageId({ image_id });
